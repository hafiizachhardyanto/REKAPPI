...
export default function DashboardPage() {
  ...
  const getStockStatus = (stock: StockGudang) => {
    const stokKG = hitungStokAkhirKG(stock);
    if (stokKG <= 0) return { label: "Habis", color: "bg-red-100 text-red-700 border-red-200" };
    if (stokKG < 1000) return { label: "Menipis", color: "bg-orange-100 text-orange-700 border-orange-200" };
    if (stokKG < 5000) return { label: "Sedang", color: "bg-yellow-100 text-yellow-700 border-yellow-200" };
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredStock.map((stock) => {
                      ...
                      const stokAwalKG = hitungStokAwalKG(stock);
                      const stokAkhirKG = hitungStokAkhirKG(stock);
                      return (
                        <tr key={stock.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="py-3 px-2 text-right">
                            <div className="font-mono text-gray-600">
                              ...
                              <span className="block text-xs text-gray-400">{stokAwalKG.toLocaleString()} KG</span>
                            </div>
                          </td>
                          <td className="py-3 px-2 text-right">
                          ...
                          <td className="py-3 px-2 text-right">
                            <div className="font-mono font-bold text-gray-800">
                              ...
                              <span className="block text-xs text-green-600">{stokAkhirKG.toLocaleString()} KG</span>
                            </div>
                          </td>
                          <td className="py-3 px-2 text-center">
                ...
                {stats.lowStock.map((stock) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-lg font-bold text-red-600">{hitungStokAkhirKG(stock).toLocaleString()} KG</p>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {fotList.map((fot) => {
                ...
                const totalKG = fotStocks.reduce((sum, s) => sum + hitungStokAkhirKG(s), 0);
                return (
                  <div
                    key={fot}
