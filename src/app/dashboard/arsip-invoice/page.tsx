...
interface ArsipInvoice {
  ...
  terbilang: string;
  riwayatPengangkutan: RiwayatPengangkutan[];
  ttdOrderNama: string;
...
export default function ArsipInvoicePage() {
  ...
  const fetchData = async () => {
    try {
      const q = query(collection(db, "arsipInvoice"), orderBy("createdAt", "desc"));
      ...
      const items = snapshot.docs.map((docSnap) => {
        ...
        return {
          ...
          riwayatPengangkutan: d.riwayatPengangkutan || [],
  ...
  const handleDeleteClick = (item: ArsipInvoice) => {
    setDeleteItem(item);
  ...
  const handlePrintInvoice = (item: ArsipInvoice) => {
    ...
    const riwayatHtml = (item.riwayatPengangkutan || []).map((r, idx) => `
      <tr>
        <td style="text-align: center; padding: 5px 3px; font-size: 9px; border: 1px solid #000;">${idx + 1}</td>
        <td style="padding: 5px 3px; font-size: 9px; border: 1px solid #000;">${r.nomorSeri}</td>
        <td style="padding: 5px 3px; font-size: 9px; border: 1px solid #000;">${r.tanggal}</td>
        <td style="padding: 5px 3px; font-size: 9px; border: 1px solid #000;">${r.driverUnit || "-"}</td>
        <td style="padding: 5px 3px; font-size: 9px; border: 1px solid #000;">${r.nomorPolisi || "-"}</td>
        <td style="text-align: right; padding: 5px 3px; font-size: 9px; border: 1px solid #000;">${r.totalKG?.toLocaleString("id-ID") || "0"} KG</td>
      </tr>
    `).join("");
    const html = `
      <!DOCTYPE html>
  ...
  const columns = [
    {
      ...
      header: "Tanggal",
      width: "120px",
      render: (row: ArsipInvoice) => <span className="font-medium text-gray-800">{row.tanggalInvoice}</span>,
    },
    {
      key: "nomorInvoice",
      header: "Nomor Invoice",
      width: "180px",
      render: (row: ArsipInvoice) => <span className="font-mono font-bold text-green-700">{row.nomorInvoice}</span>,
    },
    {
      key: "nomorPI",
      header: "Nomor PI",
      width: "150px",
      render: (row: ArsipInvoice) => <span className="font-semibold text-blue-700">{row.nomorPI}</span>,
    },
    {
      key: "namaCustomer",
      header: "Customer",
      render: (row: ArsipInvoice) => row.namaCustomer,
    },
    {
      key: "jumlahTertagih",
      header: "Jumlah",
      width: "160px",
      render: (row: ArsipInvoice) => <span className="font-mono font-medium text-gray-900">{formatRupiah(row.jumlahTertagih)}</span>,
    },
    {
      key: "aksi",
      header: "Aksi",
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <Table columns={columns} data={filteredData} isLoading={isLoading} emptyMessage="Belum ada arsip invoice" keyExtractor={(row) => row.id} onRowClick={handleDetail} />
      </Card>
        ...
        {selectedItem && (
          <div className="space-y-6">
            ...
            <div className="p-4 bg-blue-50 rounded-xl border border-blue-200">
              <p className="text-xs text-blue-600 uppercase tracking-wide font-semibold mb-3">Riwayat Pengangkutan</p>
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-blue-100">
                    <th className="px-3 py-2 text-left text-xs font-semibold text-blue-800 uppercase border">No</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-blue-800 uppercase border">Nomor Seri</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-blue-800 uppercase border">Tanggal</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-blue-800 uppercase border">Driver</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-blue-800 uppercase border">No Polisi</th>
                    <th className="px-3 py-2 text-right text-xs font-semibold text-blue-800 uppercase border">Total KG</th>
                  </tr>
                </thead>
                <tbody>
                  {(selectedItem.riwayatPengangkutan || []).map((r, idx) => (
                    <tr key={idx} className="border-b">
                      <td className="px-3 py-2 text-sm text-gray-900 border">{idx + 1}</td>
                      <td className="px-3 py-2 text-sm font-mono font-bold text-blue-700 border">{r.nomorSeri}</td>
                      <td className="px-3 py-2 text-sm text-gray-600 border">{r.tanggal}</td>
                      <td className="px-3 py-2 text-sm text-gray-600 border">{r.driverUnit || "-"}</td>
                      <td className="px-3 py-2 text-sm text-gray-600 border">{r.nomorPolisi || "-"}</td>
                      <td className="px-3 py-2 text-sm text-gray-900 text-right font-mono border">{r.totalKG?.toLocaleString("id-ID") || "0"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
